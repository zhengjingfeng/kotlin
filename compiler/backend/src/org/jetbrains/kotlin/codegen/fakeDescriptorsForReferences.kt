...
private fun TypeApproximator.approximate(descriptor: CallableMemberDescriptor, builder: CallableMemberDescriptor.CopyBuilder<*>): Boolean {
    ...
    if (returnType != null) {
        val approximatedType = approximate(returnType, toSuper = true)
    ...
    if (extensionReceiverParameter != null) {
        val approximatedExtensionReceiver = approximate(extensionReceiverParameter.type, toSuper = false)
    ...
    val newParameters = descriptor.valueParameters.map {
        val approximatedType = approximate(it.type, toSuper = false)
        if (approximatedType != null) {
...
private fun TypeApproximator.approximate(type: KotlinType, toSuper: Boolean): KotlinType? {
    if (type.arguments.isEmpty() && type.constructor.isDenotable) return null
    return if (toSuper)
        approximateToSuperType(type, TypeApproximatorConfiguration.PublicDeclaration) as? KotlinType
    else
        approximateToSubType(type, TypeApproximatorConfiguration.PublicDeclaration) as? KotlinType
}
