...
fun List<TestBuilder>.writeTestsTo(file: File, targetedSource: TargetedSourceFile<TestSourceFile>) {
    ...
    FileWriter(file).use { writer ->
        ...
        if (sourceFile in setOf(TestSourceFile.Arrays, TestSourceFile.Sequences, TestSourceFile.Collections)) {
            writer.appendLine("import test.comparisons.STRING_CASE_INSENSITIVE_ORDER")
        }
        if (sourceFile in setOf(TestSourceFile.Primitives, TestSourceFile.Unsigned, TestSourceFile.Comparisons, TestSourceFile.UComparisons, TestSourceFile.Arrays, TestSourceFile.UArrays, TestSourceFile.Collections)) {
            writer.appendLine("import kotlin.random.*")
        }
        if (sourceFile == TestSourceFile.Arrays || sourceFile == TestSourceFile.UArrays) {
            writer.appendLine("import test.assertArrayContentEquals")
        }
        if (sourceFile == TestSourceFile.Sequences) {
