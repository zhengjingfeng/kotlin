...
object OrderingTest : TestTemplateGroupBase() {
    ...
    val f_sorted = listOf("sorted", "sortedArray", "sortedDescending", "sortedArrayDescending").map { op ->
        ...
        test("$op()") {
            ...
            val sortAndCheck = "$op().iterator().assertSorted { a, b -> a $cmp b }"
            body {
                """
            ...
            bodyAppend(ArraysOfPrimitives, { it.isFloatingPoint() }) {
                """
    ...
    val f_sortBy = listOf("sortBy", "sortByDescending").map { op ->
        ...
        test("$op()") {
            ...
            body {
                ...
                data.$op { it.second }
                $assertEquals($of("ab" to 3, "aa" to 3, "aa" to 20)$reversed, data)
        
                data.$op { it.first }
                $assertEquals($of("aa" to 3, "aa" to 20, "ab" to 3)$reversed, data)
                
                data.$op { (it.first + it.second).length }
                $assertEquals($of("aa" to 3, "ab" to 3, "aa" to 20)$reversed, data)
                """
            }
        }
    }
